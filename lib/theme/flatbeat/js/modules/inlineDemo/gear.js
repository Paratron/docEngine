(function (){
    ...
    (function (){
        ...
        if(window.notice){
            html = '<div class="notice">' + window.notice + '</div>';
        }

        html += '<iframe src="' + window.demoTarget + '" id="demoFrame"></iframe>';

        demoFrame = new modo.Container();
        demoFrame.el.html(html);
        demoFrame.el.css('height', height - 3);
        demoFrame.el.children('iframe').css('height', height - 3);

        if(window.notice){
            demoFrame.el.find('.notice').on('click', function(){
                $(this).fadeOut();
            });
        }
